...
export default {
  ...
  data() {
    return {
      ...
      searchConfig: {
        ...
        CompanyNo: 'CompanyNo',
        Category: 'CategoryNo',
        Conditions: [{ Value: 'B190531005', Key: 'TypeNo', Operator: 'Equal' }]// 把表格默认参数传过去
      },
      // 表格列
      tableColumn: [
        {
        ...
        {
          prop: 'Name',
          label: this.$t('common.DepartmentName')
        },
        {
          prop: 'ChannelNo',
      ...
      tableAttr: {
        ...
        Conditions: [{ Value: 'B190531005', Key: 'TypeNo', Operator: 'Equal' }]
      },
      // 表格按钮
      tableButton: {
  ...
  methods: {
    init() {
      ...
      dataDictionary({ Data: { ParentNo: 'B190611001' }}).then(response => {
        if (response.data.MsgStatus === 1) {
    ...
    handleImport() {
      ...
      this.dialogConfig.title = this.$t('basicSettings.ImportDepartment')
