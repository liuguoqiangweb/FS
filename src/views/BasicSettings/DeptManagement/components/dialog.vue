<template>
  <el-dialog v-dialogDrag :title="dialogConfig.title" :visible.sync="dialogConfig.dialogVisible" :close-on-click-modal="false">
    <el-form ref="form" :model="companyForm" :rules="rules" label-position="right" label-width="100px" style="width: 400px; margin-left:50px;">
      <!-- 公司名称 -->
      <el-form-item :label="this.$t('common.CompanyName')" prop="CompanyNo">
        <el-select v-model="companyForm.CompanyNo" :placeholder="this.$t('common.PleaseSelect')" style="width: 100%" clearable filterable>
...
export default {
  ...
  data() {
    return {
      // 导入地址
      importUrl: this.$settings.url + 'api/BaseData/ImportBaseData',
      disabled: false,
      companyForm: {
        TypeNo: 'B190531005', // 此字段值在'数据字典-基础数据类型'获取
        CompanyNo: ''
      },
      rules: {
        CompanyNo: [
  ...
  methods: {
    ...
    beforeAvatarUpload(file) {
      // 保存导入功能额外的参数
      this.$store.commit('SET_DEFAULT_PARAMS', { CompanyNo: this.companyForm.CompanyNo, TypeNo: this.companyForm.TypeNo })
